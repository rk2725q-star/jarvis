...
class NvidiaApiClient {
  ...
  NvidiaApiClient({required String apiKey, required this.model})
    : apiKey = apiKey.trim(),
  ...
  Future<List<String>> fetchModels() async {
    try {
      ...
      if (res.statusCode != 200) {
        debugPrint(
          '[Nvidia] Models fetch failed: ${res.statusCode} - ${res.body}',
        );
        throw Exception(
          'NVIDIA model fetch failed (${res.statusCode}): ${res.body}',
        );
      }
