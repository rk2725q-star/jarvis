...
class JarvisFontLoader {
  ...
  static Future<void> loadFonts() async {
    _headingRegular = await PdfGoogleFonts.unnaRegular();   // Serif - Times New Roman style
    _headingBold    = await PdfGoogleFonts.unnaBold();
    _bodyRegular    = await PdfGoogleFonts.nunitoSansRegular(); // Sans-serif - Arial style
...
class _TextSanitiser {
  ...
  static String stripUnsupported(String s) {
    ...
    while (idx < runes.length) {
      ...
      if ((r >= 0xFE00 && r <= 0xFE0F) || r == 0xFE0F) { idx++; continue; }
      // Combining enclosing keycap (turns 0-9 into keycap emoji)
      if (r == 0x20E3) { idx++; continue; }
      // Zero-width joiners / non-joiners / BOM
      if (r == 0x200D || r == 0x200C || r == 0x200B || r == 0xFEFF) { idx++; continue; }
      // Emoji: Supplementary Multilingual Plane (U+1F000 – U+1FFFF)
      if (r >= 0x1F000 && r <= 0x1FFFF) { idx++; continue; }
      // Emoji: Extra planes (U+20000+)
      if (r >= 0x20000) { idx++; continue; }
      ...
      if (r >= 0x2600 && r <= 0x27BF) { idx++; continue; }
      // Supplemental Arrows-B, Misc Symbols and Arrows (2B00-2BFF)
      if (r >= 0x2B00 && r <= 0x2BFF) { idx++; continue; }
      // CJK Compatibility Ideographs variants & tags
      if (r >= 0xE0000 && r <= 0xE01FF) { idx++; continue; }
  ...
  static String clean(String raw) {
    ...
    s = s.replaceAllMapped(RegExp(r'\*{3}(.*?)\*{3}', dotAll: true), (m) => m.group(1) ?? '');
    // 5. Bold **text** -> text
    s = s.replaceAllMapped(RegExp(r'\*{2}(.*?)\*{2}', dotAll: true), (m) => m.group(1) ?? '');
    // 6. Italic *text* -> text
  ...
  static String _replaceSymbols(String s) {
    return s
      ...
      .replaceAll('\u00F7', '/')      .replaceAll('\u00B1', '+/-')
      .replaceAll('\u2213', '-/+')    .replaceAll('\u221E', 'inf')
      .replaceAll('\u2211', 'SUM')    .replaceAll('\u220F', 'PROD')
      .replaceAll('\u222B', 'INTG')   .replaceAll('\u222C', 'IINTG')
      .replaceAll('\u222D', 'IIINTG') .replaceAll('\u222E', 'OINT')
      .replaceAll('\u221A', 'sqrt')   .replaceAll('\u221B', 'cbrt')
      .replaceAll('\u221C', 'qrt')    .replaceAll('\u2202', 'd')
      .replaceAll('\u2207', 'nabla')  .replaceAll('\u2206', 'Delta')
      .replaceAll('\u00B0', ' deg')   .replaceAll('\u2032', "'")
      .replaceAll('\u2033', "''")     .replaceAll('\u00B2', '^2')
      .replaceAll('\u00B3', '^3')     .replaceAll('\u00BD', '1/2')
      .replaceAll('\u00BC', '1/4')    .replaceAll('\u00BE', '3/4')
      ...
      .replaceAll('\u2220', 'angle ')  .replaceAll('\u2225', 'parallel')
      .replaceAll('\u2223', 'divides ').replaceAll('\u2016', 'norm')
      .replaceAll('\u27E8', '<')       .replaceAll('\u27E9', '>')
      .replaceAll('\u2308', 'ceil(')   .replaceAll('\u2309', ')')
      .replaceAll('\u230A', 'floor(')  .replaceAll('\u230B', ')')
      ...
      .replaceAll('\u2080', '_0').replaceAll('\u2081', '_1')
      .replaceAll('\u2082', '_2').replaceAll('\u2083', '_3')
      .replaceAll('\u2084', '_4').replaceAll('\u2085', '_5')
      .replaceAll('\u2086', '_6').replaceAll('\u2087', '_7')
      .replaceAll('\u2088', '_8').replaceAll('\u2089', '_9')
      // --- Quotes / Typography ---
      .replaceAll('\u201C', '"')      .replaceAll('\u201D', '"')
      .replaceAll('\u2018', "'")      .replaceAll('\u2019', "'")
      .replaceAll('\u201A', "'")      .replaceAll('\u201E', '"')
      ...
      .replaceAll('\u25BC', 'v')      .replaceAll('\u2022', '-')
      .replaceAll('\u25CF', '-')      .replaceAll('\u2023', '-')
      .replaceAll('\u2043', '-')      .replaceAll('\u2713', 'OK')
      .replaceAll('\u2714', 'OK')     .replaceAll('\u2715', 'X')
      .replaceAll('\u2718', 'X')      .replaceAll('\u00AE', '(R)')
      .replaceAll('\u2122', '(TM)')   .replaceAll('\u00A9', '(C)')
      .replaceAll('\u2020', '+')      .replaceAll('\u2021', '++')
      .replaceAll('\u00A7', 'S.')     .replaceAll('\u00B6', 'P.')
      .replaceAll('\u2116', 'No.')    .replaceAll('\u00A0', ' ')
      ;
  }
  ...
  static List<Map<String, dynamic>> parseInline(String raw) {
    ...
    for (final m in matches) {
      if (m.start > last) {
        ...
        if (plain.isNotEmpty) result.add({'text': plain, 'bold': false, 'italic': false});
      }
      final isBoldItalic = m.group(1) != null;
      final isBold       = m.group(2) != null || m.group(4) != null;
      final isItalic     = m.group(3) != null || m.group(5) != null;
      final inner = (m.group(1) ?? m.group(2) ?? m.group(3) ?? m.group(4) ?? m.group(5) ?? '').trim();
      if (inner.isNotEmpty) {
        result.add({'text': _strip(inner), 'bold': isBold || isBoldItalic, 'italic': isItalic || isBoldItalic});
      }
      last = m.end;
    }
    if (last < text.length) {
  ...
  static String normalizeEquation(String s) {
    ...
    if (r.contains('<') && r.contains('>')) {
      ...
      r = r.replaceAllMapped(RegExp(r'<mfrac>(.*?)</mfrac>', dotAll: true), (m) {
        final inner = m.group(1) ?? '';
        final nums = RegExp(r'<mrow>(.*?)</mrow>', dotAll: true).allMatches(inner).toList();
        if (nums.length >= 2) {
          return '(${_stripXml(nums[0].group(1) ?? '')})/(${_stripXml(nums[1].group(1) ?? '')})';
        }
        return _stripXml(inner);
      });
      r = r.replaceAllMapped(RegExp(r'<msup>(.*?)</msup>', dotAll: true),
          (m) => '${_stripXml(m.group(1) ?? '')}^');
      r = r.replaceAllMapped(RegExp(r'<msub>(.*?)</msub>', dotAll: true),
          (m) => '${_stripXml(m.group(1) ?? '')}_');
      r = r.replaceAllMapped(RegExp(r'<msqrt>(.*?)</msqrt>', dotAll: true),
    ...
    r = r
      ...
      .replaceAll('StyleBox[', '')
      .replaceAll('RowBox[{', '').replaceAll('}]', '')
      .replaceAll('FullForm[', '').replaceAll('InputForm[', '');
    ...
    r = r
      ...
      .replaceAll('⌊', 'floor(').replaceAll('⌋', ')')
      .replaceAll('‖', '||').replaceAll('|', '|')
      .replaceAll('α', r'\alpha').replaceAll('β', r'\beta')
      .replaceAll('γ', r'\gamma').replaceAll('δ', r'\delta')
      .replaceAll('ε', r'\epsilon').replaceAll('ζ', r'\zeta')
      .replaceAll('η', r'\eta').replaceAll('θ', r'\theta')
      .replaceAll('λ', r'\lambda').replaceAll('μ', r'\mu')
      .replaceAll('ν', r'\nu').replaceAll('ξ', r'\xi')
      .replaceAll('π', r'\pi').replaceAll('ρ', r'\rho')
      .replaceAll('σ', r'\sigma').replaceAll('τ', r'\tau')
      .replaceAll('φ', r'\phi').replaceAll('ψ', r'\psi')
      .replaceAll('ω', r'\omega').replaceAll('Γ', r'\Gamma')
      .replaceAll('Δ', r'\Delta').replaceAll('Θ', r'\Theta')
      .replaceAll('Λ', r'\Lambda').replaceAll('Π', r'\Pi')
      .replaceAll('Σ', r'\Sigma').replaceAll('Φ', r'\Phi')
      .replaceAll('Ψ', r'\Psi').replaceAll('Ω', r'\Omega');
  ...
  static bool isMathNotation(String s) {
    ...
    if (s.contains('sqrt(') || s.contains('int_') || s.contains('sum_') ||
        s.contains('prod_') || s.contains(' oo ') || s.contains('^(')) {
      return true;
    }
  ...
  static String latexToText(String s) {
    ...
    for (int i = 0; i < 5; i++) { r = _expandFracs(r); }
    ...
    const decorCmds = [
      'mathcal', 'mathbb', 'mathbf', 'mathit', 'mathrm', 'mathfrak',
      'text', 'textbf', 'textit', 'textup', 'textrm', 'operatorname',
      'boldsymbol', 'bm', 'pmb', 'underbrace', 'overbrace',
      'overline', 'underline', 'widetilde', 'widehat',
    ];
    ...
    for (int i = 0; i < 4; i++) {
      r = r.replaceAllMapped(RegExp(r'_\{([^{}]*)\}'), (m) => '_(${m.group(1)})');
    ...
    r = r
      ...
      .replaceAll(r'\Delta', 'Delta')   .replaceAll(r'\varepsilon', 'epsilon')
      .replaceAll(r'\epsilon', 'epsilon').replaceAll(r'\zeta', 'zeta')
      .replaceAll(r'\eta', 'eta')       .replaceAll(r'\theta', 'theta')
      .replaceAll(r'\Theta', 'Theta')   .replaceAll(r'\iota', 'iota')
      .replaceAll(r'\kappa', 'kappa')   .replaceAll(r'\lambda', 'lambda')
      .replaceAll(r'\Lambda', 'Lambda') .replaceAll(r'\mu', 'mu')
      .replaceAll(r'\nu', 'nu')         .replaceAll(r'\xi', 'xi')
      .replaceAll(r'\Xi', 'Xi')         .replaceAll(r'\pi', 'pi')
      .replaceAll(r'\Pi', 'Pi')         .replaceAll(r'\varrho', 'rho')
      .replaceAll(r'\rho', 'rho')       .replaceAll(r'\varsigma', 'sigma')
      .replaceAll(r'\sigma', 'sigma')   .replaceAll(r'\Sigma', 'Sigma')
      .replaceAll(r'\tau', 'tau')       .replaceAll(r'\upsilon', 'upsilon')
      .replaceAll(r'\Upsilon', 'Upsilon').replaceAll(r'\varphi', 'phi')
      .replaceAll(r'\phi', 'phi')       .replaceAll(r'\Phi', 'Phi')
      .replaceAll(r'\chi', 'chi')       .replaceAll(r'\psi', 'psi')
      .replaceAll(r'\Psi', 'Psi')       .replaceAll(r'\omega', 'omega')
      .replaceAll(r'\Omega', 'Omega');

    // 8. Math operators
    r = r
      .replaceAll(r'\argmin', 'argmin') .replaceAll(r'\argmax', 'argmax')
      .replaceAll(r'\min', 'min')       .replaceAll(r'\max', 'max')
      .replaceAll(r'\sum', 'SUM')       .replaceAll(r'\prod', 'PROD')
      .replaceAll(r'\int', 'INTG')      .replaceAll(r'\oint', 'OINT')
      .replaceAll(r'\infty', 'inf')     .replaceAll(r'\partial', 'd')
      .replaceAll(r'\nabla', 'nabla')   .replaceAll(r'\cdot', ' * ')
      .replaceAll(r'\times', ' x ')     .replaceAll(r'\div', ' / ')
      .replaceAll(r'\pm', ' +/- ')      .replaceAll(r'\mp', ' -/+ ')
      .replaceAll(r'\leq', ' <= ')      .replaceAll(r'\geq', ' >= ')
      .replaceAll(r'\le', ' <= ')       .replaceAll(r'\ge', ' >= ')
      .replaceAll(r'\neq', ' != ')      .replaceAll(r'\ne', ' != ')
      .replaceAll(r'\approx', ' ~= ')   .replaceAll(r'\equiv', ' == ')
      .replaceAll(r'\propto', ' ~ ')
      .replaceAll(r'\rightarrow', ' -> ').replaceAll(r'\leftarrow', ' <- ')
      .replaceAll(r'\Rightarrow', ' => ').replaceAll(r'\Leftarrow', ' <= ')
      .replaceAll(r'\leftrightarrow', ' <-> ').replaceAll(r'\Leftrightarrow', ' <=> ')
      .replaceAll(r'\to', ' -> ')       .replaceAll(r'\gets', ' <- ')
      .replaceAll(r'\in', ' in ')       .replaceAll(r'\notin', ' not_in ')
      .replaceAll(r'\subset', ' subset ').replaceAll(r'\subseteq', ' ⊆ ')
      .replaceAll(r'\cup', ' ∪ ')       .replaceAll(r'\cap', ' ∩ ')
      .replaceAll(r'\forall', 'for_all ').replaceAll(r'\exists', 'exists ')
      .replaceAll(r'\neg', 'NOT ')      .replaceAll(r'\lor', ' OR ')
      .replaceAll(r'\land', ' AND ')    .replaceAll(r'\oplus', ' XOR ')
      .replaceAll(r'\log', 'log')       .replaceAll(r'\ln', 'ln')
      .replaceAll(r'\exp', 'exp')       .replaceAll(r'\sin', 'sin')
      .replaceAll(r'\cos', 'cos')       .replaceAll(r'\tan', 'tan')
      .replaceAll(r'\sinh', 'sinh')     .replaceAll(r'\cosh', 'cosh')
      .replaceAll(r'\tanh', 'tanh')
      .replaceAll(r'\lim', 'lim')       .replaceAll(r'\det', 'det')
      .replaceAll(r'\ell', 'l')         .replaceAll(r'\hbar', 'h-bar')
      .replaceAll(r'\prime', "'")       .replaceAll(r'\ldots', '...')
      .replaceAll(r'\cdots', '...')     .replaceAll(r'\vdots', ':')
      .replaceAll(r'\ddots', '...');

    // 9. Spacing → whitespace
    r = r
      .replaceAll(r'\qquad', '    ').replaceAll(r'\quad', '  ')
      .replaceAll(r'\,', ' ').replaceAll(r'\;', ' ').replaceAll(r'\:', ' ')
      .replaceAll(r'\!', '').replaceAll(r'\ ', ' ').replaceAll(r'\/', ' ');

    // 10. Remaining \command → strip backslash (keep word)
    r = r.replaceAllMapped(RegExp(r'\\([a-zA-Z]+)\*?'), (m) => m.group(1) ?? '');
  ...
  static bool isLatex(String s) {
    return s.contains(r'\frac') ||
        ...
        s.contains(r'\prod') ||
        s.contains(r'\int') ||
        s.contains(r'\min') || s.contains(r'\max') ||
        s.contains(r'\lim') ||
        s.contains(r'\mathcal') || s.contains(r'\mathbb') ||
        s.contains(r'\hat{') || s.contains(r'\bar{') ||
        s.contains(r'\theta') || s.contains(r'\alpha') ||
        s.contains(r'\beta') || s.contains(r'\sigma') ||
        s.contains(r'\epsilon') || s.contains(r'\lambda') ||
        s.contains(r'\mu') || s.contains(r'\omega') ||
        s.contains(r'\leq') || s.contains(r'\geq') ||
        s.contains(r'\nabla') || s.contains(r'\rightarrow') ||
        s.contains(r'\left') || s.contains(r'\quad') ||
        s.contains(r'\cdot') || s.contains(r'\times') ||
        s.contains(r'\sqrt') || s.contains(r'\infty') ||
...
class _Parser {
  static List<_Block> parse(String text) {
    ...
    while (i < lines.length) {
      ...
      if (t.startsWith('# ') && !t.startsWith('## ')) {
        blocks.add(_Block(type: _BlockType.h1, content: _clean(t.replaceFirst(RegExp(r'^#\s*'), ''))));
        i++;
        continue;
      }
      // H2: ## ...
      if (t.startsWith('## ') && !t.startsWith('### ')) {
        blocks.add(_Block(type: _BlockType.h2, content: _clean(t.replaceFirst(RegExp(r'^##\s*'), ''))));
        i++;
        continue;
      }
      // H3: ### ...
      if (t.startsWith('### ')) {
        blocks.add(_Block(type: _BlockType.h3, content: _clean(t.replaceFirst(RegExp(r'^###\s*'), ''))));
        i++;
        continue;
      }
      ...
      if (t.startsWith('```')) {
        ...
        if (_TextSanitiser.isLatex(normalized) || _TextSanitiser.isLatex(rawCode)) {
          ...
          blocks.add(_Block(type: _BlockType.equation, content: _TextSanitiser.cleanBlock(normalized)));
        } else {
      ...
      if (t.startsWith(r'$$')) {
        ...
        if (rest.endsWith(r'$$')) {
          ...
          blocks.add(_Block(type: _BlockType.equation,
              content: _TextSanitiser.latexToText(_TextSanitiser.normalizeEquation(raw))));
          i++;
          continue;
        }
      ...
      if (t.startsWith('|')) {
        ...
        while (i < lines.length && lines[i].trim().startsWith('|')) {
          ...
          if (!RegExp(r'^\|[-| :]+\|$').hasMatch(row)) {
            final cells = row.split('|').map((c) => _clean(c)).where((c) => c.isNotEmpty).toList();
            if (cells.isNotEmpty) rows.add(cells);
          }
          i++;
        }
        if (rows.isNotEmpty) blocks.add(_Block(type: _BlockType.table, tableData: rows));
        continue;
      }
      ...
      if (RegExp(r'^\d+[\.\)]\s').hasMatch(t)) {
        ...
        while (i < lines.length && RegExp(r'^\d+[\.\)]\s').hasMatch(lines[i].trim())) {
          items.add(_clean(lines[i].trim().replaceFirst(RegExp(r'^\d+[\.\)]\s+'), '')));
          i++;
        }
      ...
      if (_isDiagram(t)) {
        ...
        while (i < lines.length && (_isDiagram(lines[i]) || lines[i].trim().isEmpty)) {
  ...
  static bool _isDiagram(String line) {
    return line.contains('──') ||
        ...
        (line.contains('|') && (line.contains('+') && line.trimLeft().startsWith('|')));
  }
}
...
class JarvisPDFBuilder {
  ...
  static List<pw.Widget> _buildWidgets(List<_Block> blocks) {
    ...
    for (int pi = 0; pi < paired.length; pi++) {
      ...
      final isPairedHeading = (b.type == _BlockType.h2 || b.type == _BlockType.h3) &&
      ...
      if (isPairedHeading) {
        ...
        switch (nextBlock.type) {
          case _BlockType.paragraph:
            followWidgets.addAll([_buildParagraph(nextBlock.content), pw.SizedBox(height: 5)]);
            break;
          case _BlockType.bullet:
            followWidgets.addAll([_buildBullet(nextBlock.items), pw.SizedBox(height: 5)]);
            break;
          case _BlockType.numbered:
            followWidgets.addAll([_buildNumbered(nextBlock.items), pw.SizedBox(height: 5)]);
            break;
  ...
  static pw.Widget _buildParagraph(String text) => _richText(text, JarvisFontLoader.body);
        ...
        return pw.Padding(
          ...
          child: pw.Row(
            ...
            children: [
              pw.SizedBox(
                width: 22,
                child: pw.Text('${e.key + 1}.', style: JarvisFontLoader.bodyBoldStyle),
              ),
      ...
      children: [
        pw.TableRow(
          ...
          children: headers.map((h) => pw.Padding(
            ...
            child: pw.Text(h, style: JarvisFontLoader.bodyBoldStyle.copyWith(fontSize: 10), textAlign: pw.TextAlign.center),
          )).toList(),
        ),
        ...rows.map((row) => pw.TableRow(
          children: List.generate(headers.length, (i) {
            ...
            return pw.Padding(
              ...
              child: pw.Text(cell, style: JarvisFontLoader.smallBody.copyWith(fontSize: 10)),
            );
          }),
        )),
      ],
      ...
      for (final line in lines) {
        if (line.length > maxLineChars) {
          ...
          while (remaining.length > maxLineChars) {
            wrapped.add(first ? remaining.substring(0, maxLineChars) : '  ${remaining.substring(0, maxLineChars)}');
            remaining = remaining.substring(maxLineChars);
            first = false;
          }
          if (remaining.isNotEmpty) wrapped.add(first ? remaining : '  $remaining');
        } else {
          wrapped.add(line);
        }
      }
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              leading: const Icon(Icons.download_rounded, color: Color(0xFF7C83FD)),
              title: const Text('Download PDF', style: TextStyle(color: Colors.white)),
              subtitle: const Text('Save to Downloads folder', style: TextStyle(color: Colors.white54, fontSize: 12)),
              onTap: () { Navigator.pop(ctx); _executePDF(download: true); },
            ),
            ListTile(
              leading: const Icon(Icons.share_rounded, color: Color(0xFF4DE8B2)),
              title: const Text('Share PDF', style: TextStyle(color: Colors.white)),
              subtitle: const Text('Share via WhatsApp, Drive, Gmail...', style: TextStyle(color: Colors.white54, fontSize: 12)),
              onTap: () { Navigator.pop(ctx); _executePDF(download: false); },
            ),
            const SizedBox(height: 16),
          ],
        ),
