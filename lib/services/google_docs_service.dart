...
class GoogleDocsService {
        ...
        docs.Request(
          updateTextStyle: docs.UpdateTextStyleRequest(
            textStyle: docs.TextStyle(
              fontSize: docs.Dimension(magnitude: 8, unit: 'PT'),
              italic: true,
            ),
      ...
      case SegmentType.divider:
        requests.add(
          docs.Request(
            updateTextStyle: docs.UpdateTextStyleRequest(
              textStyle: docs.TextStyle(
                fontSize: docs.Dimension(magnitude: 6, unit: 'PT'),
    ...
    for (var line in lines) {
      ...
      if (t.startsWith('```')) {
        if (!inDiagram) {
          ...
          inDiagram = false;
          segments.add(
            DocSegment(SegmentType.diagram, diagramBuf.toString().trimRight()),
          );
        }
        continue;
      }
  ...
  docs.Request _paraStyleReq(
    int s,
    int e, {
    String named = 'NORMAL_TEXT',
    double spaceAbove = 0,
    double spaceBelow = 0,
    double indentStart = 0,
    double indentEnd = 0,
  }) {
    return docs.Request(
  ...
  docs.OptionalColor _grey() => docs.OptionalColor(
...
''';
    ...
    final fullOutline = await _generateText(
      outlinePrompt,
      system: "You are an academic researcher.",
    );
    final sections = fullOutline
        .split('\n')
        .where(
          (l) =>
              l.trim().isNotEmpty && (l.contains('.') || l.contains('Section')),
        )
        .toList();
    ...
    fullDocumentBuffer.writeln(
      'This document provides an exhaustive, multi-chapter analysis of $topic, covering technical, philosophical, and practical dimensions across $minPages+ pages.\n',
    );
    ...
    for (var i = 0; i < sections.length; i++) {
      ...
      onStatus?.call(
        'Writing Chapter ${i + 1}/${sections.length}: $sectionTitle',
      );
...
''';
      ...
      final sectionContent = await _generateText(
        expandPrompt,
        system:
            "Write detailed academic content. Aim for maximum technical density.",
      );
  ...
  Future<String> _generateText(String prompt, {String? system}) async {
    ...
    if (ollamaCloud != null) {
      try {
        ...
        final res = await ollamaCloud!.chat(
          messages: messages,
    ...
    throw Exception(
      'No AI model client available or failed in JarvisDocAgent.',
    );
  }
}
